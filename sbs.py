...
class SBS():
    ...
    if is_root() == False:
        ...
        utils.LOG_DEBUG("Process is running as administrator/root. No need to elevate.")
    ...
    def processRedditFeed(self, update_frequency):
        ...
        for post in d.entries:
            if self.isProfane(post.title):
                ...
                self.utils.LOG_INFO(f"NOT ADDING: '{post.title}' as it is profane.")
            else:
                post_index += 1
                print(f"ADDING: '{post.title}' as it is NOT profane.")
                self.utils.LOG_INFO(f"ADDING: '{post.title}' as it is NOT profane.")
                self.create_tts_file(post.title, os.getcwd(), f"polly_out{post_index}.mp3")
        thoughts = ['ShowerThoughts Database File:', f"{self.vars.currentDate}--{self.vars.current_time}"]
        for post in d.entries:
            thoughts.append(post.title)
        thoughts.append("That's all for today! Come back tomorrow for more ShowerThoughts.")
        self.utils.create_file_from_path(f"{os.getcwd()}/thoughts/")
    ...
    def make_polly_file(self, out_dir_var):
        ...
        self.utils.LOG_DEBUG("Running command: '"+ merge_command + "'")
        file_names = ""
        for tts_file in tts_files:
        ...
        self.utils.LOG_DEBUG("Running command: '"+ cleanup_command + "'")

        move_command = f"mv polly_output{self.vars.currentId}.mp3 {self.vars.out_dir}"
        self.utils.LOG_DEBUG("Running command: '"+ move_command + "'")
    ...
    def create_rss_feed(self):
        ...
        tts_filesize = self.utils.get_file_size(self.vars.out_dir + self.vars.out_file)
    ...
    def main(self):
        version_URL = urllib.request.urlopen(f"{self.main_url}ST_VERSION")
        ...
        self.vars.createEnvVars()
        if currentOS == 0:
            self.utils.create_file_from_path("/var/www/html/st/polly_out/")
        elif currentOS == 1:
            ...
            print("Your OS, Darwin (AKA: macOS or Mac OS X), is currently not supported. Exiting gracefully...")
            self.utils.LOG_ERROR(
